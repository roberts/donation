...
- **Logic**:
    ...
    - **New**: Inertia-based Admin Panel or Filament (if preferred, but Inertia specified).

### 3. API
- **Route**: `/api/*`
- **Logic**:
    - Read-only access to Schools and Donations.
    - Supports filtering (`?col=val`), sorting, and pagination.
    - **Legacy**: ArrestDB (`api/index.php`).
    - **New**: Laravel API Resources + Controllers (`Api\SchoolController`, `Api\DonationController`).
...
- **Logic**:
    ...
    - **New**: `barryvdh/laravel-dompdf` or `spatie/laravel-pdf` with Blade templates.

### 6. Email Notifications
- **Logic**:
    - Sends receipt PDF to donor via email.
    - Uses AWS SES (implied by legacy config).
    - **Legacy**: SwiftMailer.
    - **New**: Laravel Mailables + SES Driver.
